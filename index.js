...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName == "login") {
    interaction.reply({ content: "تم تسجيل الدخول بنجاح" });
  }
});
...
client.on(`interactionCreate`, (interaction) => {
  ...
  if (interaction.commandName == "setup") {
    ...
    const embed = new MessageEmbed().setDescription(
      "لشراء اعضا يجب عليك فتح تكت اولا"
    );
...
client.on(`interactionCreate`, async (interaction) => {
  ...
  if (interaction.customId == "openticket") {
    const ceatogry = await interaction.guild.channels.cache.get(
      config.bot.ceatogry
    );

    const ChannelSpin = await interaction.guild.channels.create(
    ...
    const embed = new MessageEmbed().setDescription(
      "انقر علي الزر بالاسفل لشراء اعضاء"
    );
    ...
    await interaction.reply({
      content: `تم انشاء تذكرة شراء الاعضاء `,
      ephemeral: true,
    });
  }
});
...
client.on(`interactionCreate`, async (interaction) => {
  ...
  if (interaction.customId == "buyMembers") {
    const BuyModal = new Modal().setCustomId("BuyModal").setTitle("شراء اعضاء");
...
client.on(`interactionCreate`, async (interaction) => {
  ...
  if (interaction.customId == "BuyModal") {
    ...
    if (!guild)
      return interaction
        .reply({ content: `**عذرًا , لم اتمكن من العثور على الخادم ..**` })
        .catch(() => {
    ...
    if (amount > alld.length)
      return interaction
        .reply({ content: `**لا يمكنك ادخال هاذا العدد ..**` })
        .catch(() => {
          interaction.channel.send({
            content: `**لا يمكنك ادخال هاذا العدد ..**`,
          });
        });
